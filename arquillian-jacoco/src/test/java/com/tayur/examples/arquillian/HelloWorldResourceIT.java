...
public class HelloWorldResourceIT {
    ...
    public void shouldBeAbleToConnect() throws Exception {
        ...
        WebTarget webTarget = client.target(deploymentURL.toString() + "api/world/continents");

        Invocation.Builder invocationBuilder = webTarget.request(MediaType.APPLICATION_JSON);

        Response response = invocationBuilder.get();

        assertEquals("Expected status code to be 200 OK", Response.Status.OK.getStatusCode(), response.getStatus());
    }
